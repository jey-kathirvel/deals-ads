...
implements ProviderDealMapper {
  ...
  map(
    ...
    const title =
      this.firstText(
        element,
        AmazonSelectorProfile.title,
      );

    const currentPrice =
      this.firstMoney(
        element,
        AmazonSelectorProfile.currentPrice,
      );

    const originalPrice =
      this.firstMoney(
        element,
        AmazonSelectorProfile.originalPrice,
      ) ?? currentPrice;

    const imageUrl =
      this.firstAttribute(
        element,
        AmazonSelectorProfile.image,
        "src",
      ) ?? "";
    ...
    const discountFromPage =
      this.firstPercentage(
        element,
        AmazonSelectorProfile.discount,
      );
