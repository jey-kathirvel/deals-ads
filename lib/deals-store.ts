...
export async function cleanupGroceryDeals() {
  ...
  for (const deal of deals) {
    ...
    const expired = deal.status === "expired" || hasExpired(deal.expiryDate);

    const inactivePublished = deal.status === "published" && !deal.active;
...
export async function saveDeal(
  input: Partial<Deal> &
    Pick<Deal, "title" | "platform" | "category" | "price" | "mrp" | "url">,
) {
  ...
  const deal: Deal = {
    ...
    tag: input.tag?.trim() || "New deal",
    color: input.color || "#e7f1ec",
    emoji: input.emoji || "DEAL",
    imageUrl: input.imageUrl?.trim() || "",
    code: input.code?.trim() || "",
    expires: input.expires?.trim() || "Limited time",
    url: affiliateUrl(input.url.trim(), input.platform),
    active: input.active ?? existing?.active ?? true,
    source: input.source || existing?.source || "manual",
    status: input.status || existing?.status || "published",
    expiryDate: input.expiryDate || existing?.expiryDate || "",
    couponTerms: input.couponTerms?.trim() || existing?.couponTerms || "",
    sourceUrl:
      input.sourceUrl?.trim() || existing?.sourceUrl || input.url.trim(),
    providerItemId: input.providerItemId?.trim() || existing?.providerItemId,
    providerPlatform:
      input.providerPlatform?.trim() || existing?.providerPlatform,
    lastCheckedAt: input.lastCheckedAt || new Date().toISOString(),
    importedAt:
      existing?.importedAt || input.importedAt || new Date().toISOString(),
...
function isPublicDeal(item: Deal) {
  return (
    item.active && item.status === "published" && !hasExpired(item.expiryDate)
  );
}
...
export async function importDeals(
  ...
  options: { publish?: boolean; source?: Deal["source"] } = {},
) {
  if (rows.length === 0) {
    return {
      imported: 0,
  ...
  const seenTitles = new Set(
    items.map(
      (item) =>
        `${item.platform.toLowerCase()}|${item.title.toLowerCase().replace(/\W/g, "")}`,
    ),
  );
  const imported: Deal[] = [];
  const errors: string[] = [];
  ...
  for (let index = 0; index < rows.length; index++) {
    ...
    if (seenUrls.has(canonical(url)) || seenTitles.has(titleKey)) {
      skipped++;
      continue;
    }
    const deal = await saveDeal({
      ...
      tag: row.tag || "Imported deal",
      code: row.code || row.coupon || "",
      couponTerms: row.couponTerms || row.terms || "",
      expires: row.expires || "Limited time",
      expiryDate: row.expiryDate || "",
      rating: Number(row.rating || 4.5),
      votes: Number(row.votes || 0),
      active: options.publish ?? false,
      status: options.publish ? "published" : "review",
      source: options.source || "csv",
      sourceUrl: row.sourceUrl || url,
      providerItemId: row.providerItemId || "",
      providerPlatform: row.providerPlatform || platform,
    });
    imported.push(deal);
    seenUrls.add(canonical(deal.url));
...
export async function markDealsChecked(
  ids: readonly number[],
  checkedAt?: string,
) {
  return markLegacyDealsChecked(ids, checkedAt);
}
